<?php 
  // Prepare query with bind variable to prevent SQL injection
  $query = "SELECT * FROM Admin WHERE admin_id = :aid";
  $stmt = oci_parse($conn, $query);
  
  // Bind the session variable
  oci_bind_by_name($stmt, ":aid", $_SESSION['userId']);
  oci_execute($stmt);
  
  // Fetch result using Oracle's default uppercase column keys
